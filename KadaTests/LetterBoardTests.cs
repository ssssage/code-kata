...
namespace KadaTests
{
    public class LetterBoardTests
	{
		...
		
	    {
			...
			{
			return initialLeaderBoardResult.OrderByDescending(initialLeaderBoardResult => initialLeaderBoardResult.Score).Select((initialLeaderBoardResult, index) => new FinalLeaderBoardResult { InitialLeaderBoardResult = initialLeaderBoardResult, Rank = index + 1});
			}
		}
